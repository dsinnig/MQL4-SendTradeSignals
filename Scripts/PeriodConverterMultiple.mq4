...
#property description "Period Converter to update format of history base"
#property strict
#property show_inputs

input string multipliers = "5M_15M_30M_1H_4H_1D"; // Period multiplier factor
int ExtHandle=-1;
int periodMultipliersInt[20];
string periodMultipliersString[20];

int stringToMultiplier(string timeframe) {
   if (StringCompare(timeframe, "5M") == 0) return 5;
   if (StringCompare(timeframe, "15M") == 0) return 15;
   if (StringCompare(timeframe, "30M") == 0) return 30;
   if (StringCompare(timeframe, "1H") == 0) return 60;
   if (StringCompare(timeframe, "4H") == 0) return 240;
   if (StringCompare(timeframe, "1D") == 0) return 1440;
   return 0;
}
...
void OnStart()
  {
   ...
   int numberOfTimeframes=StringSplit(multipliers,u_sep,periodMultipliersString);
   for (int i = 0; i < numberOfTimeframes; ++i) {
      periodMultipliersInt[i] = stringToMultiplier(periodMultipliersString[i]);
   }
   
   for (int k = 0; k < numberOfTimeframes; ++k) {
      Print(periodMultipliersString[k]);
      datetime time0;
      ulong    last_fpos=0;
      long     last_volume=0;
   ...
   //---- History header
      ...
      int      i_period=Period()*periodMultipliersInt[k];
      int      i_digits=Digits;
      int      i_unused[13];
      MqlRates rate;
   //---  
